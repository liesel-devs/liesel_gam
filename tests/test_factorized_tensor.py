...
def _term(basis: jax.Array, name: str) -> gam.StrctTerm:
...
def _explicit_basis(*bases: jax.Array) -> jax.Array:
...
def test_factorized_three_way_is_jittable_and_has_no_full_design_intermediate():
    ...
    def evaluate(beta: jax.Array) -> jax.Array:
...
def test_indexed_marginal_uses_gather_and_retains_its_penalty():
    ...
    dense = _term(jnp.column_stack((jnp.ones(n), x, x**2, x**3)), "x")
