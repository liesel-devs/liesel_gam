...
def test_two_piece_student_t_df_weights_are_accurate_under_jit():
    df = jnp.array([14.0, 15.0, 16.0, 1_000.0, 100_000.0])
    # Fixed high-precision evaluations of the exact Fisher-information formula.
    expected = jnp.array(
        [
            1.3966154418072602e-2,
            1.2353779316987623e-2,
            1.1005490597457852e-2,
            3.487039381362402e-6,
            3.499870003949881e-10,
        ]
    )
    model = DictModel()
    weights_fn = IWLSWeights.two_piece_student_t_df()

    def jit_weights(value):
        return weights_fn(model, {"df": value})  # type: ignore

    actual = jax.jit(jit_weights)(df)

    assert jnp.allclose(actual[0], expected[0], rtol=5e-4, atol=0.0)
    assert jnp.allclose(actual[1:], expected[1:], rtol=5e-6, atol=0.0)
